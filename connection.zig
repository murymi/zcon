...
pub const Connection = struct {
    ...
    pub fn executeQuery(self: *Self, query: [*c]const u8, parameters: anytype) ![]u8 {
        const stmt = try self.prepareStatement(self.mysql, query);
        ...
        const resultBuffers = try self.bindResultBuffers(stmt,columns, columnCount);
        const res = try self.fetchResults(stmt, columns, columnCount, resultBuffers);
        //resultBuffers.
        return res;
    }
    ...
    pub fn prepareStatement(self: *Self,mysql: *c.MYSQL, query: [*c]const u8) !*c.MYSQL_STMT {
        _ = self;
    ...
    pub fn bindResultBuffers(self: *Self,statement: *c.MYSQL_STMT, columns: [*c]c.MYSQL_FIELD,columnCount: usize) !*Bufflist {
        var result_bind: [*c]c.MYSQL_BIND = @as([*c]c.MYSQL_BIND, @ptrCast(@alignCast(c.malloc(@sizeOf(c.MYSQL_BIND) *% @as(c_ulong, columnCount)))));
        const blist = try Bufflist.init(self.allocator, @as(usize, columnCount));
    ...
    pub fn fetchResults(self: *Self,statement: *c.MYSQL_STMT,columns: [*c]c.MYSQL_FIELD,columnCount: usize,resultBuffers: *Bufflist) ![]u8 {
        ...
        while (c.mysql_stmt_fetch(statement) == @as(c_int, 0)) {
            try wr.beginObject();
            for (0..columnCount) |v| {
                const czstr = try cStringToZigString((columns.?[v]).name, self.allocator);
        ...
        //const fstr = try blist.getBufferAsString(0);
